...
**Read the current file before editing this step.** Task 3's own fix round (already complete and reviewed) added a `FlattenedItem = FlattenedPrimitive | OpaqueMeshPrimitive` discriminated union to this file — a mesh with an array `material` bypasses grouping entirely as an `{kind: "opaque", chunkId, mesh}` item, while a normal single-material mesh becomes `{kind: "groupable", chunkId, geometry, material, matrix, transparent}` — plus a `mergeCompatibleSingletons` pass that explicitly excludes both transparents and opaque nodes. The restructuring below preserves that discriminated union and every behavior Task 3's review verified (multi-material bypass, transparent exclusion, per-chunk isolation); it only splits resolution+flattening from grouping so seam removal can run between them, and exposes live per-cell `THREE.Mesh` objects for it to mutate. Do not revert to a plain (non-discriminated) `FlattenedPrimitive` shape — verify your diff still has the `kind` field and the opaque-node handling before moving on.

Modify `src/environment/geometryCompiler.ts` so resolution+flattening (stages 1-2) and grouping (stages 3-4) are two separately callable functions, with `flattenRecord` changed to return both the flattened items (groupable + opaque) and the live mesh nodes backing them (seam removal, in Task 4, mutates a mesh's `geometry` in place — since a `FlattenedPrimitive`'s `geometry` field and an `OpaqueMeshPrimitive`'s `mesh.geometry` are the *same object reference* as the traversed node's `geometry`, a mutation is visible everywhere without any copying step). Replace the file's exported surface with:
...
/** Stage 3-4 (post seam-removal): instances/merges already-flattened items into render nodes, scoped per chunk. Body is the existing grouping/opaque-handling/merge logic from `compileGeometry`, unchanged — only its inputs move from a freshly-resolved `items` array to the `itemsByRecordId` map `flattenRecords` produced. */
...
Change `flattenRecord`'s return type and body to also collect the live mesh nodes traversed, alongside the existing groupable/opaque item split (the `if (Array.isArray(node.material))` branch and the `items.push({kind: "groupable", ...})` branch are unchanged from the current file — only the added `meshes.push(node)` line and the return shape are new):
...
Delete the old standalone `compileGeometry` function body (the one written in Task 3, whose logic is now split between `groupPrimitives` and the thin wrapper above) — do not delete `mergeCompatibleSingletons` or `groupBy`, which stay as module-private helpers used by `groupPrimitives`.

Note on seam removal and re-reading geometry: after `removeInternalSeamFaces` mutates `mesh.geometry` (via `geometry.setIndex(...)`, which mutates the existing `BufferGeometry` instance rather than replacing it), both a `FlattenedPrimitive`'s `geometry` field and an `OpaqueMeshPrimitive`'s `mesh.geometry` still point at the same, now-updated `BufferGeometry` — no re-sync step is needed between seam removal and `groupPrimitives`.
...
- **Ground geometry generation and full attribute-complete vertex welding** — spec's geometry compiler stages 5 ("Split the ground into chunk-sized quads") and 8 (welding vertices that match across all relevant attributes). Neither was implemented in this plan's 8 tasks — `flattenRecords` only walks `recipe.cells`/`recipe.objects`, so the exported GLB contains no ground mesh even though `manifestBuilder.ts` emits a `ground` manifest record referencing real chunk IDs. This was an omission surfaced by this plan's final whole-branch review, not a deliberate deferral made during planning — flagged here explicitly, after the fact, so it reads as a known, scoped-out gap rather than a silent one. A follow-up plan must either implement both stages before the feature is considered complete, or the design/spec must be amended if ground geometry turns out to be unnecessary for the first release.
- **Seam-removal instancing loss and per-record bounds fidelity** are both real, lower-severity gaps surfaced by the same review and intentionally left for a follow-up plan rather than fixed here: (1) when `removeInternalSeamFaces` is enabled, cells now clone their own geometry before mutation (fixed in this plan's final-review fix wave, see below) — this is correct but means seam-processed cells lose automatic instancing with same-asset cells that weren't touched; a future optimization could re-group by post-mutation geometry content instead of pre-mutation identity. (2) `manifestBuilder.ts`'s per-record `bounds` are computed from `cellSize`/`scale` alone (a synthetic footprint), not from the record's actual compiled geometry — `buildManifestRecords` currently has no access to compiled geometry by design (it only consumes `SceneRecipe`/`ChunkAssignment`/the asset table, deliberately independent of live Three.js objects). Giving it real bounds would require threading compiled-geometry bounds through the orchestrator into the manifest builder, a real but bounded architecture change better done as its own reviewed task than folded into a bug-fix pass.
- **Seam-removal matching performance** — `seamRemoval.ts`'s boundary-triangle matching is a nested per-mesh-pair scan recomputed from scratch each time, which is adequate for this plan's own tests but not evaluated against the spec's 100x100-grid performance target. Left as a known scaling risk for a later profiling pass, not fixed here.
