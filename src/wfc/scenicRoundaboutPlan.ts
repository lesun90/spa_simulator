...
export function planRoundabout(exits: readonly PlanarDirection[], palette: PlanarWfcPalette, seed: number, recipe: ScenicRecipe = scenicRecipeForPalette(palette)): readonly SolvedPlanarCell[] {
  ...
  for (let row = 0; row < 3; row++) for (let column = 0; column < 3; column++) {
    ...
    const id = island ? recipe.roundabout.island : corner ? recipe.roundabout.corner : exits.includes(outward!) ? recipe.roundabout.entrance : recipe.roundabout.closedExit;
    ...
    const allowed = palette.variants.filter((variant) => scenicAssetId(variant.assetId, recipe) === id
  ...
  for (const cell of result.cells) for (const direction of planarDirections) {
    ...
    const reference = palette.variants.find((v) => scenicAssetId(v.assetId, recipe) === (road ? recipe.straightRoad : recipe.ground)
