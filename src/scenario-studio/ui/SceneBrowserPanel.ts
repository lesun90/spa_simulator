...
export class SceneBrowserPanel extends BasePanel {
  ...
  constructor(
    ...
    this.root.add(this.heading.root, this.refreshButton.root, this.divider.root, this.scroll.root, this.empty.root);
  }

  setChoices(choices: readonly SceneChoice[]): void {
    this.choices = choices;
    this.empty.setText(choices.length ? "" : "No published scenes found in assets/scenes.");
    this.rebuild();
  }
  ...
  private rebuild(): void {
    ...
    for (const row of this.rows) { row.tile.dispose(); row.texture?.dispose(); }
    this.rows = [];
    ...
    const layout = layoutAssetTiles({ assetCount: this.choices.length, grid, tile: tileSize });
    this.choices.forEach((choice, index) => {
      ...
      void this.thumbnails.getStaticThumbnail(choice).then((texture) => {
        if (generation !== this.refreshGeneration) return;
        row.texture = texture;
        tile.setThumbnailTexture(texture);
        this.onVisualChange();
      }).catch(() => {});
    });
  ...
  private endHover(choice: SceneChoice): void {
    ...
    if (row) void this.thumbnails.getStaticThumbnail(choice).then((texture) => row.tile.setThumbnailTexture(texture));
  }
